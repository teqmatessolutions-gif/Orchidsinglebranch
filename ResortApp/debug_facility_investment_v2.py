...
def debug_facility_capital_investment():
    ...
    try:
        ...
        total_sum = 0
        print(f"{'Date':<12} | {'Item':<20} | {'Qty':<5} | {'Price':<10} | {'Total':<10}")
        print("-" * 70)
        
        for detail, master, item in details:
            print(f"{str(master.purchase_date):<12} | {item.name[:20]:<20} | {detail.quantity:<5} | {detail.unit_price:<10} | {detail.total_amount:<10}")
            total_sum += detail.total_amount
            
        print("-" * 70)
        print(f"Total Capital Investment (Sum of Purchases): {total_sum}")
